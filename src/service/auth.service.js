...
class AuthenticationService {
    ...
    if (type === "list") {
      ...
      try {
        const user = await this.AdminModel.create({
          ...
          password: hashedPassword,
          image:
            "https://res.cloudinary.com/dvznn9s4g/image/upload/v1734708983/avatar_i1xry2.png",
          privilege,
        });
        await this.sendEmailVerificationCode(user.emailAddress, user.id, type);
      ...
      variables: {
        resetLink:
          serverConfig.NODE_ENV === "development"
            ? `http://localhost/COMPANYS_PROJECT/ResetPassword/sendPasswordLink.html?${params.toString()}`
            : `${serverConfig.DOMAIN}/adminpanel/Passwor?${params.toString()}`,
      },
    });
  }
    ...
    try {
      if (transactionStatus.paymentReference.startsWith("appointmentAndRent")) {
        ...
        if (transactionStatus.paymentStatus == "PAID") {
          ...
          if (
            TenantModelResult.paymentReference ===
  ...
  async disburseRent(BuildingModel, prospectiveTenantId) {
    ...
    await this.TransactionModel.create({
      ...
      amount: userService.calculateDistribution(
  ...
  async handleIntializePayment(data) {
    try {
      ...
      if (
        ...
        "There's no transaction matching supplied reference. Please confirm supplied reference and try again."
      ) {
  ...
  async getRegion() {
    try {
      ...
      const buildingPreferences =
        typeof setting.preferences === "string"
          ? JSON.parse(setting.preferences)?.region
          : setting.preferences.region;
  ...
  async getMaritalStatus() {
    try {
      ...
      const maritalStatus =
        typeof setting.preferences === "string"
          ? JSON.parse(setting.preferences)?.maritalStatus
          : setting.preferences.maritalStatus;
  ...
  async getReligion() {
    try {
      ...
      const religion =
        typeof setting.preferences === "string"
          ? JSON.parse(setting.preferences)?.religion
          : setting.preferences.religion;
  ...
  async getGender() {
    try {
      ...
      const gender =
        typeof setting.preferences === "string"
          ? JSON.parse(setting.preferences)?.gender
          : setting.preferences.gender;
  ...
  async getAcctBalance(accountNumber) {
    try {
      ...
      const response = await axios.get(
        `${serverConfig.MONNIFY_BASE_URL}/api/v1/disbursements/wallet/balance`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
            Accept: "application/json",
  ...
  async getTransactionStatus(transactionReference) {
    ...
    try {
      ...
      const response = await axios.get(
        `${serverConfig.MONNIFY_BASE_URL}/api/v2/merchant/transactions/query?transactionReference=${encodedTransactionReference}`,
        {
          headers: {
  ...
  async getRefundTransactionStatus(refundReference) {
    ...
    try {
      ...
      const response = await axios.get(
        `${serverConfig.MONNIFY_BASE_URL}/api/v1/refunds/${refundReference}`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
            "Content-Type": "application/json",
  ...
  async getTransactionStatusDisbursement(Reference) {
    ...
    try {
      const response = await axios.get(
        `${serverConfig.MONNIFY_BASE_URL}/api/v2/disbursements/single/summary?reference=${Reference}`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
            "Content-Type": "application/json",
  ...
  async authorizeSingleTransfer(reference, authorizationCode) {
    ...
    try {
      const response = await axios.post(
        `${serverConfig.MONNIFY_BASE_URL}/api/v2/disbursements/single/validate-otp`,
        {
          reference,
          authorizationCode,
        },
  ...
  async getTransactionStatus2(transactionReference) {
    ...
    try {
      ...
      const response = await axios.get(
        `${serverConfig.MONNIFY_BASE_URL}/api/v2/disbursements/single/summary?reference=${encodedTransactionReference}`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
            "Content-Type": "application/json",
  ...
  async getTransactionStatusSingleTransfer(transactionReference) {
    ...
    try {
      const response = await axios.get(
        `${serverConfig.MONNIFY_BASE_URL}/api/v2/disbursements/single/summary?transactionReference=${transactionReference}`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
            "Content-Type": "application/json",
  ...
  async updateTransactionStatusCronJobSingleTransfer(transaction, status) {
    try {
      ...
      if (
        status == "SUCCESS" &&
        transaction.transactionType == "commissionOrRent"
      ) {
        const TenantModelResult = await this.TenantModel.findOne({
          where: {
            buildingId: transaction.buildingId,
  ...
  async handleUploadPicture(data, file) {
    ...
    try {
      ...
      if (serverConfig.NODE_ENV == "production") {
        accessPath = serverConfig.DOMAIN + file.path.replace("/home", "");
      } else if (serverConfig.NODE_ENV == "development") {
        accessPath = serverConfig.DOMAIN + file.path.replace("public", "");
      }
      await user.update({ image: accessPath, isImageVerified: true });
  ...
  async handleLoginAdmin(data) {
    const { emailOrTel, password } =
      await authUtil.verifyHandleLoginAdmin.validateAsync(data);
  ...
  async handleVerifyEmailorTelAdmin(data) {
    let { userId, verificationCode, who, type } =
      await authUtil.verifyHandleVerifyEmailorTelAdmin.validateAsync(data);

    let relatedEmailoRTelValidationCode = "";

    if (who == "business") {
      relatedEmailoRTelValidationCode =
        await this.EmailandTelValidationBusinessModel.findOne({
          where: {
            userId: userId,
      ...
      relatedEmailoRTelValidationCode =
        await this.EmailandTelValidationAdminModel.findOne({
          where: {
            userId: userId,
            verificationCode: verificationCode,
            type,
      ...
      relatedEmailoRTelValidationCode =
        await this.EmailandTelValidationBusinessSpotModel.findOne({
          where: {
            userId: userId,
            verificationCode: verificationCode,
            type,
    ...
    try {
      ...
      try {
        const options = {
          ...
          headers: {
            "X-RapidAPI-Key":
              "b03b577e45mshb62f4d5ecbfae57p108324jsna07cb1377bb8",
  ...
  async startFirstRentDisbursements() {
    try {
      ...
      const retryTimeInSeconds = settings?.failedDisburseRetry
        ? parseInt(settings.failedDisburseRetry)
        : 1800; // Default to 1800 seconds if not found
      const pendingDisburseRetry = settings?.pendingDisburseRetry
        ? parseInt(settings.pendingDisburseRetry)
        : 1800;
  ...
  async processDisbursement(propertyManager, inspection) {
    try {
      ...
      if (
        ...
        inspection.landlordPaidStatus === false
      ) {
        ...
        propertyManager.type === "agent" &&
        inspection.agentPaidStatus === false
      ) {
  ...
  async rentDisbursements() {
    ...
    for (let index = 0; index < TenantModelResult.length; index++) {
      ...
      for (const unpaidTransaction of TransactionModelResult) {
        if (
          unpaidTransaction.paymentStatus === TRANSACTION_STATUS.ABANDONED ||
  ...
  async checkRefund() {
    try {
      const settings = await this.SettingModel.findOne(); // Fetch settings from the database
      const retryDelay = settings?.failedRefundRetry
        ? settings.failedRefundRetry
        : 1800; // Default to 1800 seconds if not found
      ...
      const inspectionsToRefund = await this.InspectionModel.findAll({
        where: {
          ...
          [Op.or]: [{ propertyManagerStatus: false }, { tenentStatus: false }],
          isDeleted: false,
        },
      });
      ...
      for (const inspection of inspectionsToRefund) {
        ...
        if (refundExists) {
          ...
          if (
            refundStatus === TRANSACTION_STATUS.ABANDONED ||
    ...
    try {
      ...
      await this.RefundLogModel.create({
        ...
        refundTransactionReference:
          "refund_inspection" + "_" + transactionReference,
        inspectionId: inspection.id,
        buildingId: inspection.buildingId,
  ...
  async initiateRefund(refundMetaData, authToken) {
    ...
    try {
      const refundResponse = await axios.post(
        `${serverConfig.MONNIFY_BASE_URL}/api/v1/refunds/initiate-refund`,
        refundPayload,
        {
          headers: {
